...
const outputPath = 'd:/RJR_tamil_website_01/rjr_tamil_website_main_01/rjr_tamil/app/data/branches_full.json';
...
try {
    ...
    const enrichedData = branchesRaw.map(region => {
        const branches = region.branches.map(b => {
            const city = b.city;
            const state = region.state || region.name;
    ...
    console.log("Successfully created branches_full.json");
