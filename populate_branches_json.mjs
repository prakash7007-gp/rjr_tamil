...
const branchesRaw = JSON.parse(fs.readFileSync(inputPath, 'utf-8'));
...
// Start a counter for doctor rotation
let doctorIndex = 0;
...
const enrichedData = branchesRaw.map(region => {
    const branches = region.branches.map(b => {
        ...
        // Using the embed URL format we want in the static JSON
        ...
        return {
            ...
            city: city,
            url: b.url, // Keeping regular URL for reference if needed
            address: address,
            phone: phone,
            mapUrl: mapUrl,
    ...
    return {
        state: region.state, // Ensure consistent naming 'state' vs 'name'
...
console.log("Updated branches.json with full data.");
